...
describe('EventsContainer', () => {
  ...
  describe('getCachedPastEvents', () => {
    ...
    it('in test mode calls the API', async () => {
      ...
      await getCachedPastEvents()
      expect(fetch.mock.calls.length).toEqual(1)
      expect(fetch.mock.calls[0][0]).toEqual('https://hjoutysc5k.execute-api.eu-west-1.amazonaws.com/test/get-past-events')
    })
    ...
    it('in prod mode calls the API', async () => {
      ...
      await getCachedPastEvents()
      expect(fetch.mock.calls.length).toEqual(1)
      expect(fetch.mock.calls[0][0]).toEqual('https://267sder6c7.execute-api.eu-west-1.amazonaws.com/prod/get-past-events')
    })
