...
const Clients = (props: ClientsProps) => {
  ...
  return (
    <div className="mt-3">
      <GridList cellHeight={200} cols={isSmall ? 2 : 4} className={props.classes.gridList}>
        {(isSmall ? clientsListSmallOrder : clientsList).map(client => {
          return (
            <GridListTile key={client.name} cols={client.featured ? 2 : 1}>
              {client.featured ? (
                <div className="row justify-content-center align-items-center mx-0" style={{ width: '100%', height: '100%' }}>
                  <div className={props.classes.background}>
                    <ResponsiveImage path={client.background} alt={client.name} width="100%" height="100%" style={{ objectFit: 'cover' }} />
                  </div>
                  <div className={`${props.classes.faded} ${props.classes.background}`} />
                  <div className="col-8 col-md-6">
                    <ResponsiveImage path={client.logo} alt={client.name} width="100%" />
                  </div>
                </div>
              ) : (
                <div
