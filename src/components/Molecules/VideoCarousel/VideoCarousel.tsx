...
import { leftControls, rightControls, bottomControls } from '../TeamCarousel/renderControls'
  ...
  return (
    ...
    >
      {props.videos.map(video => (
        <Card key={video.id} className={`${styles.card} my-3`} onClick={() => props.onSelectVideo(video)}>
          <CardMedia className={styles.cardMedia} image={video.thumbnails.medium.url} title={video.title} />
          <CardContent>
            <Typography variant="headline" component="h3" title={video.title}>
              <Truncate text={video.title} limit={40} />
            </Typography>
            <p className={styles.publishedAt}>{formatDate(video.publishedAt, i18n.language)}</p>
          </CardContent>
        </Card>
      ))}
    </Carousel>
...
const Truncate: SFC<TruncateProps> = ({ limit, text }) => {
  const textToRender = text.length > limit ? `${text.substr(0, limit)}...` : text
  return <>{textToRender}</>
}
