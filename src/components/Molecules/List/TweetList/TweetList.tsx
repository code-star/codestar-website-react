...
import { Card, CardContent, CardActions, Button, CardMedia, CardHeader, Avatar, Typography } from '@material-ui/core'
...
const TweetList = ({ tweets, eventDate, eventImage, eventName, children }: ITweetListProps) => {
  ...
  return (
    <Card raised className={styles.card}>
      {firstTweetArr.map(tweet => (
        <CardHeader
          key={tweet.id}
          avatar={<Avatar src={tweet.user.profile_image_url_https} aria-label="Speaker Twitter Avatar" />}
          ...
          <div key={tweet.id} className="p-2">
            <a href={`https://twitter.com/${tweet.user.screen_name}/status/${tweet.id_str}`}>
              <Typography component="div" className={styles.prefix}>
                <img src={getResponsiveImageUrl('/images/events/twitter', 30)} className="mr-2" alt="twitter event" />
      ...
      <CardActions>
        {children}
        {firstTweetArr.map(tweet => (
          <Button key={tweet.id} size="small" color="secondary" href={`https://twitter.com/${tweet.user.screen_name}?lang=en`}>
