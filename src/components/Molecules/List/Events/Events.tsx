...
const Events: SFC<IEventInnerProps & IEventOuterProps> = ({
  ...
  const pastEventsList = pastMeetupEvents.map((mEvent: IMeetupEvent) => <EventCard key={mEvent.time} event={mEvent} />)
  return (
    <>
      <EventsHeader nextMeetupEvents={nextMeetupEvents} noNextMeetupEvent={noNextMeetupEvent} tweets={recentTweets} />
      <Section scrollname="previous-events">
        <Container>
          <NextEventsBlock events={nextMeetupEvents} nextEventsTitle={t('OUR_NEXT_EVENTS')} />
...
export default compose<IEventInnerProps & IEventOuterProps, IEventOuterProps>(translate(['events'], { wait: true }))(Events)
