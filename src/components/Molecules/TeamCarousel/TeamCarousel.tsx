...
const TeamCarousel: SFC = () => {
  return (
    ...
    >
      {shuffleArray(Team)
        .filter((person: any) => person && !person.gone && person.name && person.image)
        .map((person: any) => (
          <Card key={person.image} className={`${styles.card} my-3`}>
            <CardMedia
              className={styles.cardMedia}
              image={getResponsiveImageUrl(`/images/team/${person.image}`, cardWidth * 2, 'e_grayscale/co_rgb:0057ae,e_colorize:40')}
              title={person.name}
            />
            <CardContent>
