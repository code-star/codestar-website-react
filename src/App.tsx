...
import AsyncComponent, { ComponentTypePromise } from './AsyncComponent/AsyncComponent'
...
function fullHeightAsyncComponent<Props>(component: ComponentTypePromise) {
  return (props: Props) => <AsyncComponent fullHeight component={() => component} {...props} />
}

const AsyncIntro = fullHeightAsyncComponent(import('./Intro/Intro'))
const AsyncCases = fullHeightAsyncComponent(import('./Cases/Cases'))
const AsyncAbout = fullHeightAsyncComponent(import('./components/Pages/About/About'))
const AsyncJobs = fullHeightAsyncComponent(import('./Jobs/Jobs'))
const AsyncJobDescription = fullHeightAsyncComponent<JobDescriptionOuterProps>(import('./JobDescription/JobDescription'))
...
const AsyncEvents = fullHeightAsyncComponent(import('./containers/EventsContainer/EventsContainer'))
...
class App extends Component<AppProps, AppState> {
  ...
  constructor(props: AppProps) {
    ...
    this.history.listen(location => this.updateBackgroundColor(location.pathname))
        ...
        <MuiThemeProvider theme={theme}>
          ...
          <ScrollToTop>
            <Switch>
              ...
              {jobsList.map(job => (
                <Route exact path={`/jobs/${job.path}`} key={job.path} render={() => <AsyncJobDescription {...job} />} />
              ))}
