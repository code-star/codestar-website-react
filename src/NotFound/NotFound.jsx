...
const styles = theme => ({
  ...
  container: {
    ...
    overflowY: 'hidden',
    alignItems: 'center',
    maskImage: 'linear-gradient(to bottom, rgba(0, 0, 0, 0.2) 0%, rgba(0, 0, 0, 1) 40%, rgba(0, 0, 0, 1) 60%, rgba(0, 0, 0, 0.2) 100%)',
    paddingTop: '48px',
...
class NotFound extends Component {
  render() {
    ...
    const textTop = data.substring(0, data.length / 2 + length - ((data.length / 2) % length))
    const textBottom = data.substring(data.length / 2)

    const notFoundTextOffsets = notFoundTexts.map(text => text.length + (length - (text.length % length)))
    ...
    function hexEditorify(text, initialIndex, styleClass) {
      return [...Array(Math.ceil(text.length / length)).keys()].map(i => {
        ...
        return (
          <pre style={{ margin: 0 }} key={initialIndex + i} className={`m-0 ${styleClass} ${classes.hexLine}`}>
            0x
    ...
    const hexEditorCodeTop = hexEditorify(textTop, offset - textTop.length, classes.otherLines)

    const hexEditorCodeMiddle = notFoundTexts.map((text, i) =>
      hexEditorify(text, i > 0 ? offset + notFoundTextOffsets[i - 1] : offset, classes.mainLine)
    )

    const hexEditorCodeBottom = hexEditorify(textBottom, offset + notFoundTextOffset, classes.otherLines)
