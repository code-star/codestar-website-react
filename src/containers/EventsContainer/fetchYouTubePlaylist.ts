...
export async function fetchYouTubePlaylist(apiKey: string, playlistId: string): Promise<VideoItem[]> {
    ...
    .then(items =>
      items.map(
        (item: any): VideoItem => ({
          ...
          description: item.snippet.description ? item.snippet.description.split('\n') : [],
          thumbnails: item.snippet.thumbnails,
        })
      )
    )
    .then((videos: VideoItem[]) => videos.sort((a, b) => b.publishedAt.valueOf() - a.publishedAt.valueOf()))
}
